    ...
    do
    {
        ...
        switch (choice)
        {
            ...
            case 2: 
                ...
                displayStudents(studentArray, size);
                break;

            case 3:
                //Search stuent by ID
                foundStudent = searchStudent(studentArray, size);

                if (foundStudent != NULL)
                {
                    printf("  Name: %s\n", foundStudent->name); 
                    printf("  ID: %d\n", foundStudent->id);
                    printf("  GPA: %.2f", foundStudent->gpa);
                    printf("\n");
                } else {
...

{
    ...
    //asks for user input and checks for valid input
    do {
    
        printf("Enter Student VIN: ");
        status = scanf("%d", &vinSearch); //status will reciece 1 if input was an int
        printf("\n");
